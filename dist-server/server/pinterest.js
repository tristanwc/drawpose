export async function fetchUserBoards(bookmark, token) {
    if (!token)
...
async function scrapeBoardId(boardUrl) {
    ...
    if (!res.ok)
        throw new Error(`Failed to fetch board page: ${res.status}`);
    ...
    if (!match)
        throw new Error('Could not find initialReduxState in board HTML');
    let data;
    try {
...
export async function fetchBoardPins(url, bookmark, boardId, token) {
    if (!token)
        throw new Error('Not authenticated');
    ...
    const data = await res.json();
    const images = (data.items ?? []).flatMap(pin => {
